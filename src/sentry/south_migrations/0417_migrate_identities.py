...
class Migration(DataMigration):
    ...
    def _forwards(self, orm):
        ...
        for (typ, ext_id), idps in grouped_idps.items():
            new_idp = orm.IdentityProvider(type=typ, organization_id=0, external_id=ext_id)
            for idp in idps:
            ...
            for f in filtered:
                ...
                if key in new_ids:
                    eid = new_ids[key]
                    assert eid.user_id == f.user_id and eid.external_id == f.external_id
                    eid.date_verified = max(eid.date_verified, f.date_verified)
                    eid.date_added = max(eid.date_added, f.date_added)
                else:
                    new_ids[key] = orm.Identity(
                        idp=new_idp,
                        user_id=f.user_id,
                        external_id=f.external_id,
                        status=0,  # IdentityStatus.VALID
                        date_verified=f.date_verified,
                        date_added=f.date_added,
                    )

        print("%d new IDPs and %d new IDs" % (len(new_idps), len(new_ids)))
